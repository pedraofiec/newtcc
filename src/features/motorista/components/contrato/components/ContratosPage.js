...
import {
  listarContratosPorMotorista,
  listarContratos,
  deletarContrato,
} from "../service/ContratosService";
...
const ContratosPage = () => {
  ...
  async function carregar() {
    ...
    if (!motoristaId) {
      console.warn(
        "motoristaId ausente no localStorage — tentando listar todos os contratos como fallback."
      );
      try {
  ...
  function tratarErro(err, context = "") {
    ...
    if (err?.response) {
      ...
      setServerError({
        status: 0,
        body: "Sem resposta do servidor (network/CORS?)",
        context,
      });
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <div>
          <button
            onClick={() => navigate("/driver/contratos/novo")}
      ...
      {serverError && (
        <div className="mb-4 p-4 bg-red-50 border border-red-200 rounded">
          <strong className="text-red-700">
            Erro {serverError.status || ""} no servidor
          </strong>
          <div className="text-sm text-red-600 mt-2">
            ...
            <ul className="list-disc ml-5 mt-2">
              ...
              <li>
                Verificar o console do navegador para ver o body da resposta do
                servidor.
              </li>
            </ul>
          </div>
          ...
          <div className="mt-3 flex gap-2">
            ...
            <button
              onClick={() => {
                console.log("Server error body:", serverError.body);
                alert(JSON.stringify(serverError.body, null, 2));
              }}
      ...
      {contratos.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {contratos.map((c) => {
            // 👇 PRIORIDADE: nome do dependente/criança
            const nomePassageiro =
              c.dependenteNome || // campo que mandamos no payload
              c.nomeDependente ||
              c.nomeCrianca ||
              c.alunoNome ||
              c.aluno?.nome ||
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">
                      Início: {c.dataInicio} • Fim: {c.dataFim}
                    </p>
                    <p className="text-sm text-gray-600">
                  ...
                  <div className="flex flex-col gap-2">
                    <button
                      onClick={() =>
                        navigate(`/driver/contratos/${c.id}/editar`)
                      }
